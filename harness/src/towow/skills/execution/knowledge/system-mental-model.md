...
所有 commit 走同一条命令 `uv run --directory harness python -m towow.cli.main submit`——这条命令在 M-3.1 submit wrapper 进程内运行。
...
- 调 `uv run --directory harness python -m towow.cli.main submit` 提交 envelope
