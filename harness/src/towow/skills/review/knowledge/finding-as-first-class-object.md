...
closure_contract:
  closure_criteria:                         # 必填——"什么样才算 finding 修完"
    - condition: string                     # 可验证条件
      ...
      expected_occurrences: int             # grep 必填: 门 grep 出的真 count 比较基准（比较语义见下）
      occurrence_comparator: enum [eq | gte]  # 可选, 缺省 eq（向后兼容）。condition 是"精确计数"
        #   （通常 0，如"残留必须清零"）用缺省 eq；condition 是"存在性"（"至少引用了 X"/"包含了 Y"）
        #   必须显式填 gte（下限而非精确目标）——否则 f-ohr3-done-criteria-exact-match-vs-existence-intent
        #   同型病复发：精确相等承载存在性意图，真 occurrence 数随文档措辞漂移即物理无法闭合。
